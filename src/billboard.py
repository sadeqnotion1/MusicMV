...
class ChartData:
    ...
    def _parseNewStylePage(self, soup):
        ...
        for entrySoup in soup.select("ul.o-chart-results-list-row"):
            ...
            if imageElement:
                image = imageElement.get("data-lazy-src") or imageElement.get("src")
            else:
                image = None
        ...
        if len(self.entries) > 0:
            ...
            for entry in self.entries:
                if entry.title and entry.artist:
                    ...
                    a = entry.artist.lower().strip()
                    if t == a:
                        num_duplicated += 1
                elif not entry.title or entry.title == "":
                    num_duplicated += 1
            
            if num_duplicated >= len(self.entries) * 0.9:
                for entry in self.entries:
                    if entry.title and entry.artist:
                        t = entry.title.lower().strip()
                        a = entry.artist.lower().strip()
                        if t == a:
                            if entry.title != entry.artist and entry.artist.lower() == entry.title.lower():
                                pass
                            else:
...
class ArtistHistoryEntry:
    """Represents a historical entry for an artist on the Hot 100.
    """

    def __init__(
        self, title, artist, debut_date, peak_pos, peak_weeks, peak_date, weeks_on_chart
    ):
        self.title = title
        self.artist = artist
        self.debut_date = debut_date
        self.peak_pos = peak_pos
        self.peak_weeks = peak_weeks
...
class ArtistHistory:
    """Represents the complete Hot 100 history for an artist.
    """

    def __init__(self, artist_slug, entries, stats):
        self.artist_slug = artist_slug
        self.entries = entries
        self.stats = stats

    def __repr__(self):
        return "{}.{}(artist_slug={!r}, num_songs={!r})".format(
            self.__class__.__module__, self.__class__.__name__, self.artist_slug, len(self.entries)
        )
...
def get_artist_history(artist_slug, max_retries=5, timeout=25):
    """GETs the artist's Hot 100 chart history from Billboard.com, then parses
    the data using BeautifulSoup.
    """
    # Clean and format slug to Billboard standards (e.g. "Sabrina Carpenter" -> "sabrina-carpenter")
    import unicodedata
    import re
    nfkd_form = unicodedata.normalize('NFKD', artist_slug)
    ascii_slug = "".join([c for c in nfkd_form if not unicodedata.combining(c)])
    # Replace dots, spaces, dollars, and ampersands with hyphens
    slug = ascii_slug.lower().strip()
    slug = re.sub(r'[\s\.\$&@]+', '-', slug)
    # Remove any other non-alphanumeric/non-hyphen characters
    slug = re.sub(r'[^a-z0-9\-]', '', slug)
    # Collapse multiple hyphens and strip leading/trailing hyphens
    cleaned_slug = re.sub(r'-+', '-', slug).strip('-')
    url = "https://www.billboard.com/artist/%s/chart-history/hsi/" % (cleaned_slug)
    session = _get_session_with_retries(max_retries=max_retries)
    req = session.get(url, timeout=timeout)
    if req.status_code == 404:
        raise BillboardNotFoundException("Artist not found or has no Hot 100 history.")
    req.raise_for_status()

    soup = BeautifulSoup(req.text, "html.parser")
    
    # Parse high-level summary cards (No 1 Hits, Songs Count, Top 10 Hits)
    stat_0 = soup.select_one(".artist-stat-0")
    stat_1 = soup.select_one(".artist-stat-1")
    stat_2 = soup.select_one(".artist-stat-2")
    
    stats = {
        "no1_hits": stat_0.text.strip() if stat_0 else "—",
        "songs": stat_1.text.strip() if stat_1 else "—",
        "top10_hits": stat_2.text.strip() if stat_2 else "—"
    }

    entries = []
    for row in soup.select(".o-chart-results-list-row"):
        try:
            title_el = row.select_one(".artist-chart-row-title")
            title = title_el.text.strip() if title_el else ""

            artist_el = row.select_one(".artist-chart-row-artist")
            artist = artist_el.text.strip() if artist_el else ""

            debut_el = row.select_one(".artist-chart-row-debut-date")
            debut_date = debut_el.text.strip() if debut_el else ""

            peak_el = row.select_one(".artist-chart-row-peak-pos")
            peak_pos = int(peak_el.text.strip()) if peak_el else None

            week_el = row.select_one(".artist-chart-row-peak-week")
            peak_weeks = week_el.text.strip() if week_el else ""

            peak_dt_el = row.select_one(".artist-chart-row-peak-date")
            peak_date = peak_dt_el.text.strip() if peak_dt_el else ""

            weeks_el = row.select_one(".artist-chart-row-week-on-chart")
            weeks_on_chart = int(weeks_el.text.strip()) if weeks_el else None
    ...
    return ArtistHistory(artist_slug, entries, stats)
